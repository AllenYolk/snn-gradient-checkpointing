...
class CIFAR10DVS(sjds.NeuromorphicDatasetFolder):
    ...
    def read_aedat_save_to_np(bin_file: str, np_file: str):
        ...
        np.savez(np_file, t=events["t"], x=events["x"], y=events["y"], p=events["p"])
...
class CIFAR10DVSDataModule(L.LightningDataModule):
    ...
    def prepare_data(self):
        frame_root = Path(self.data_dir) / f"frames_number_{self.T}_split_by_number"
        ...
        if not train_set_root.exists() or not test_set_root.exists():
            # split the dataset
            move_data(Path(self.data_dir) / f"frames_number_{self.T}_split_by_number")
    ...
    def setup(self, stage: str):
        ...
        self.train_set = TransformedDatasetWrapper(train_set, transform=transform_train)
        self.test_set = TransformedDatasetWrapper(test_set, transform=transform_test)
